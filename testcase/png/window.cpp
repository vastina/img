...
int main()
{
  ...
  vastina::png::png p { "./test/window.png", ww, hh };

  while ( !quit ) {
    ::system( "rm -f ./test/window.png" );
    ...
    for ( unsigned y = 0; y < p.height; y++ ) {
      for ( unsigned x = 0; x < p.width; x++ ) {
        // put rgb inside the loop, you will see what old TVs will show to you
        // p.setIndex( x, y, { r, g, b } );
        unsigned rr = x * x + y * y;
        if ( rr > rr1 && rr < rr2 )
    ...
    p.write();
    SDL_Surface* loadedSurface = IMG_Load( "./test/window.png" );
